    ...
    def process(self):
        ...
        if self.filtered_googleFit_activitiesData.empty:
            ...
            return pd.DataFrame(columns=[
                'userName', 'valueGeneratedAt', 'Sport', 'Lap.StartTime', 'Lap.Track.Trackpoint.Time', 'unit', 'duration', 'Lap.Calories'
            ])

        # Convert Lap.TotalTimeSeconds to numeric
        ...
        grouped_data = self.filtered_googleFit_activitiesData.groupby('Lap.StartTime').agg(
            ...
            caloriesBurned=('Lap.Calories', 'first'),
        ...
        final_googleFit_activitiesData = grouped_data[['userName', 'valueGeneratedAt', 'Sport', 'Lap.StartTime', 'startDate', 'endDate', 'unit', 'duration', 'caloriesBurned']]

        # Round duration and Lap.Calories
        final_googleFit_activitiesData['duration'] = final_googleFit_activitiesData['duration'].round(1)
        final_googleFit_activitiesData['caloriesBurned'] = pd.to_numeric(final_googleFit_activitiesData['caloriesBurned']).round(1)
